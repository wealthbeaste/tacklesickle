(() => {
  ...
  async function renderEvents() {
    ...
    if(items.length){h+='<div class="table-wrap"><table class="data-table"><thead><tr><th>Event</th><th>District</th><th>Date</th><th>Team Lead</th><th>Partners</th><th>Actions</th></tr></thead><tbody>';items.forEach(e=>{h+='<tr><td><strong>'+esc(e.event_name)+'</strong></td><td>'+esc(e.district)+'</td><td>'+fmtDate(e.event_date)+'</td><td>'+esc(e.team_lead||'\u2014')+'</td><td>'+esc(e.partners||'\u2014')+'</td><td><button class="btn-secondary" style="padding:5px 8px;font-size:.78rem;color:#dc2626" onclick="window.__regDeleteEvent('+e.id+')"><i class="fa-solid fa-trash"></i></button></td></tr>';});h+='</tbody></table></div>';h+='<div class="mobile-cards">';items.forEach(e=>{h+='<div class="record-card"><div class="card-name">'+esc(e.event_name)+'</div><div class="card-meta"><span><i class="fa-solid fa-location-dot"></i> '+esc(e.district)+'</span><span><i class="fa-solid fa-calendar"></i> '+fmtDate(e.event_date)+'</span><span><i class="fa-solid fa-user"></i> '+esc(e.team_lead||'\u2014')+'</span></div>'+(e.partners?'<p style="font-size:.85rem;color:var(--reg-muted);margin-top:6px">Partners: '+esc(e.partners)+'</p>':'')+'<div class="card-actions"><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem;color:#dc2626" onclick="window.__regDeleteEvent('+e.id+')"><i class="fa-solid fa-trash"></i> Delete</button></div></div>';});h+='</div>';}else h+='<div class="empty-state"><p>No events created yet.</p></div>';
    h+='</div>';mainContent.innerHTML=h;}catch(e){mainContent.innerHTML='<div class="reg-alert reg-alert-error">'+esc(e.message)+'</div>';}
  }
  ...
  async function renderPendingReview() {
    ...
    items.forEach(s=>{h+='<tr><td>'+esc(s.tsca_id)+'</td><td>'+esc(s.first_name)+' '+esc(s.last_name)+'</td><td>'+fmtDate(s.screening_date)+'</td><td>'+esc(s.test_type)+'</td><td>'+rBadge(s.result)+'</td><td>'+esc(s.screening_site||'\u2014')+'</td><td><button class="btn-secondary" style="padding:6px 10px;font-size:.8rem" onclick="window.__regReviewScreening('+s.id+',\'VALIDATED\')"><i class="fa-solid fa-check"></i> Approve</button> <button class="btn-secondary" style="padding:6px 10px;font-size:.8rem;color:#dc2626" onclick="window.__regReviewScreening('+s.id+',\'REJECTED\')"><i class="fa-solid fa-xmark"></i> Reject</button> <button class="btn-secondary" style="padding:6px 10px;font-size:.8rem;color:#dc2626" onclick="window.__regDeleteScreening('+s.id+')"><i class="fa-solid fa-trash"></i></button></td></tr>';});
    h+='</tbody></table></div><div class="mobile-cards">';
    items.forEach(s=>{h+='<div class="record-card"><div class="card-header"><div><span class="card-id">'+esc(s.tsca_id)+'</span><div class="card-name">'+esc(s.first_name)+' '+esc(s.last_name)+'</div></div>'+rBadge(s.result)+'</div><div class="card-meta"><span><i class="fa-solid fa-flask"></i> '+esc(s.test_type)+'</span><span><i class="fa-solid fa-calendar"></i> '+fmtDate(s.screening_date)+'</span><span><i class="fa-solid fa-location-dot"></i> '+esc(s.screening_site||'\u2014')+'</span></div><div class="card-actions"><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem" onclick="window.__regReviewScreening('+s.id+',\'VALIDATED\')"><i class="fa-solid fa-check"></i> Approve</button><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem;color:#dc2626" onclick="window.__regReviewScreening('+s.id+',\'REJECTED\')"><i class="fa-solid fa-xmark"></i> Reject</button><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem;color:#dc2626" onclick="window.__regDeleteScreening('+s.id+')"><i class="fa-solid fa-trash"></i> Delete</button></div></div>';});
    h+='</div>';}else h+='<div class="empty-state"><p>No screenings pending review.</p></div>';
    h+='</div>';mainContent.innerHTML=h;}catch(e){mainContent.innerHTML='<div class="reg-alert reg-alert-error">'+esc(e.message)+'</div>';}
  }
  ...
  async function renderAdminUsers() {
    ...
    h+='</tbody></table></div><div class="mobile-cards">';
    users.forEach(u=>{h+='<div class="record-card"><div class="card-header"><div><span class="card-id">'+esc(u.username)+'</span><div class="card-name">'+esc(u.full_name)+'</div></div>'+roleBadge(u.role)+'</div><div class="card-meta"><span><i class="fa-solid fa-circle-'+(u.status==='ACTIVE'?'check':'xmark')+'"></i> '+esc(u.status)+'</span><span><i class="fa-solid fa-clock"></i> '+(u.last_login_at?fmtDate(u.last_login_at):'Never')+'</span></div>'+(u.role!=='ADMINISTRATOR'?'<div class="card-actions"><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem" onclick="window.__regToggleUser('+u.id+',\''+u.status+'\')">'+(u.status==='ACTIVE'?'Disable':'Enable')+'</button><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem;color:#dc2626" onclick="window.__regDeleteUser('+u.id+')">Delete</button></div>':'')+'</div>';});
    h+='</div></div>';mainContent.innerHTML=h;}catch(e){mainContent.innerHTML='<div class="reg-alert reg-alert-error">'+esc(e.message)+'</div>';}
  }
  ...
  async function renderRegistrations(page) {
    ...
    h+='<div class="table-wrap"><table class="data-table"><thead><tr><th>Registry #</th><th>Name</th><th>Email</th><th>Phone</th><th>Type</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>';
    items.forEach(r=>{h+='<tr><td><strong>'+esc(r.registry_number)+'</strong></td><td>'+esc(r.full_name)+'</td><td>'+esc(r.email)+'</td><td>'+esc(r.phone||'\u2014')+'</td><td><span class="badge badge-'+esc(r.subscription_type)+'">'+esc(r.subscription_type)+'</span></td><td><span class="badge badge-'+(r.status==='active'?'active':'inactive')+'">'+esc(r.status)+'</span></td><td>'+fmtDate(r.created_at)+'</td><td><button class="btn-secondary" style="padding:5px 8px;font-size:.78rem;color:#dc2626" onclick="window.__regDeleteRegistration('+r.id+')"><i class="fa-solid fa-trash"></i></button></td></tr>';});
    h+='</tbody></table></div><div class="mobile-cards">';
    items.forEach(r=>{h+='<div class="record-card"><div class="card-header"><div><span class="card-id">'+esc(r.registry_number)+'</span><div class="card-name">'+esc(r.full_name)+'</div></div><span class="badge badge-'+esc(r.subscription_type)+'">'+esc(r.subscription_type)+'</span></div><div class="card-meta"><span><i class="fa-solid fa-envelope"></i> '+esc(r.email)+'</span><span><i class="fa-solid fa-phone"></i> '+esc(r.phone||'\u2014')+'</span><span><i class="fa-solid fa-calendar"></i> '+fmtDate(r.created_at)+'</span></div><div class="card-actions"><button class="btn-secondary" style="padding:8px 14px;font-size:.85rem;color:#dc2626" onclick="window.__regDeleteRegistration('+r.id+')"><i class="fa-solid fa-trash"></i> Delete</button></div></div>';});
